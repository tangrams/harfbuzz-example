...
class HBShaper {
public:
	HBShaper(const string& fontFile, FontLib<FF,FG>* lib);
	~HBShaper();
...
template <typename FF, typename FG>
HBShaper<FF,FG>::HBShaper(const string& fontFile, FontLib<FF,FG>* fontLib) {
	lib = fontLib;
    face = lib->loadFace(fontFile, 50 * 64, 72, 72);
}

template <typename FF, typename FG>
void HBShaper<FF,FG>::drawText(HBText& text) {
